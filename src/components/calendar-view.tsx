...
const CalendarView = ({ selectedDate, onDateSelect, workouts, currentWeek, onWeekChange, onMonthChange }: CalendarViewProps) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <Button variant="ghost" size="icon" onClick={() => onMonthChange('prev')}>
          <ChevronLeft className="h-5 w-5" />
        </Button>
        <h2 className="text-xl font-semibold">
          {format(selectedDate, 'MMMM yyyy')}
        </h2>
        <Button variant="ghost" size="icon" onClick={() => onMonthChange('next')}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center justify-center space-x-1 sm:space-x-2 md:space-x-3">
          {days.map(day => {
            ...
            return (
              <div key={day.toString()} className="flex flex-col items-center space-y-1 sm:space-y-2">
                ...
                <Button
                  ...
                  className={cn(
                    'w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full flex flex-col relative shrink-0',
